 ...
 * pared de Bogotá.
 *
 * Pensado para `inventario_respuestas.fecha_confirmacion`, una columna
...
/**
 * Fecha de hoy en Bogotá, formato `YYYY-MM-DD` (sin hora) -- para comparar
 * contra `calendario_inventario.fecha_asignada` (columna `date`). Mismo
 * truco de desplazamiento que `nowBogotaNaiveIso`, solo que se descarta la
 * parte de hora.
 */
export function todayBogotaDateString(): string {
  const bogotaShifted = new Date(Date.now() + BOGOTA_OFFSET_MS);
  return bogotaShifted.toISOString().slice(0, 10);
}
